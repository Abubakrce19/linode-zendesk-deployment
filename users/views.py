...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user:
                if user.is_active:  #                 if not user.is_active:
    ...
    return render(request, 'users/sign_in.html', {'form': form, 'error': error, 'SOCIALS': True})
...
def registeration_view(request):
    if request.method == 'POST':
        ...
        form = RegisterForm()
    return render(request, 'users/sign_up.html', {'form': form, 'SOCIALS': True})
